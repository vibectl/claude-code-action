...
await check("Import: github/constants", async () => {
	...
	assert(
		mod.CLAUDE_APP_BOT_ID !== undefined,
		"CLAUDE_APP_BOT_ID not exported",
	);
	assert(
		mod.CLAUDE_BOT_LOGIN !== undefined,
		"CLAUDE_BOT_LOGIN not exported",
	);
});
...
await check("@actions/github resolvable", async () => {
	...
	assert(
		...
		"@actions/github.context not available",
	);
});
...
await check("zod resolvable", async () => {
	...
	assert(
		typeof mod.z?.string === "function",
		"zod.z.string not a function",
	);
});
